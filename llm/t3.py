...
def process_specific_folder(downloads_path: str, folder_name: str, references_path: str = "References", device: str = "cuda"):
    """处理指定文件夹中的所有PDF文件"""
    ...
    print(f"\nFound {len(pdf_files)} PDF files in folder: {folder_name}")
    ...
    # 创建输出CSV文件
    combined_csv_file = os.path.join(references_path, f"{folder_name}_references.csv")
    
    if os.path.exists(combined_csv_file):
        os.remove(combined_csv_file)
        print(f"Removed existing file: {combined_csv_file}")
    
    print(f"Results will be saved to: {combined_csv_file}")
    ...
    print(f"Results saved to: {combined_csv_file}")
    
    if total_successful > 0:
        try:
            with open(combined_csv_file, 'r', encoding='utf-8') as f:
                line_count = sum(1 for line in f) - 1  # 减去表头
            print(f"Final CSV contains: {line_count} references")
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="Batch process academic papers for reference extraction")
    parser.add_argument("--downloads_path", default="Downloads", help="Path to Downloads folder containing paper folders")
    ...
    parser.add_argument("--folder_name", help="Process a specific folder name within Downloads path")
    parser.add_argument("--debug", action="store_true", help="Debug folder structure without processing")
    parser.add_argument("--batch_cvpr", action="store_true", help="Batch process all CVPR folders (original behavior)")
    ...
    if args.single_pdf:
        ...
        success = process_specific_folder(args.downloads_path, args.folder_name, args.references_path, args.device)
        if success:
            print(f"\nSuccessfully processed folder: {args.folder_name}")
        else:
            print(f"\nFailed to process folder: {args.folder_name}")
    
    elif args.batch_cvpr:
        ...
        print("  --single_pdf: Process a single PDF file")
        print("  --folder_name: Process a specific folder")
        print("  --batch_cvpr: Process all CVPR folders")
        print("  --debug: Debug folder structure")
        print("\nExamples:")
        print("  python t2.py --folder_name NeurIPS")
        print("  python t2.py --folder_name ICLR")
        print("  python t2.py --batch_cvpr")
        print("  python t2.py --debug")

if __name__ == "__main__":
    main()


# python t2.py --folder_name NeurIPS --downloads_path ~/hpc-share/revolutionizing-higher-ed-rankings/Downloads/ --references_path ~/hpc-share/revolutionizing-higher-ed-rankings/References/
